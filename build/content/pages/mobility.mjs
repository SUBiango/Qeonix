...
const CAPS = [
  ...
  { icon: "vehicle", h: T("Fleet orchestration", "تنسيق الأساطيل"), p: T("Telemetry, utilisation, condition and predictive maintenance across public and commercial fleets, with dispatch tied to demand.", "القياس والاستخدام والحالة والصيانة التنبؤية عبر الأساطيل العامة والتجارية، مع إرسال مرتبط بالطلب.") },
  { icon: "radar", h: T("Intelligent transportation systems", "أنظمة النقل الذكية"), p: T("Corridor management, signal optimisation and incident response built on live network state rather than last year's counts.", "إدارة الممرّات وتحسين الإشارات والاستجابة للحوادث بناءً على حالة الشبكة المباشرة، لا على إحصاءات العام الماضي.") },
...
const STACK = [
  { label: T("Traveller experience", "تجربة المتنقّل"), note: T("The journey as one product.", "الرحلة كمنتج واحد."), items: [T("Journey planning", "تخطيط الرحلات"), T("Booking & ticketing", "الحجز والتذاكر"), T("Unified payment", "الدفع الموحّد"), T("Real-time information", "المعلومات اللحظية"), T("Accessibility", "إمكانية الوصول")] },
  ...
  { label: T("Transport intelligence", "ذكاء النقل"), note: T("The network, understood.", "الشبكة، مفهومة."), items: [T("Network state", "حالة الشبكة"), T("Demand forecasting", "التنبؤ بالطلب"), T("Incident detection", "كشف الحوادث"), T("Signal optimisation", "تحسين الإشارات"), T("Corridor analytics", "تحليلات الممرّات")] },
  ...
  { label: T("Integration & control", "التكامل والتحكم"), note: T("Where operators and city meet.", "حيث يلتقي المشغّلون بالمدينة."), items: [T("Operator APIs", "واجهات المشغّلين"), T("Payment providers", "مزوّدو الدفع"), T("City command centre", "مركز قيادة المدينة"), T("Enforcement systems", "أنظمة الضبط"), T("Open data", "البيانات المفتوحة")] },
];
...
const JOURNEY = [
  ...
  { label: T("Adapt", "التكيّف"), icon: "radar", note: T("Disruption rerouted in minutes, with the traveller told first.", "إعادة توجيه عند الاضطراب خلال دقائق، ويُخطَر المتنقّل أولًا.") },
...
const PRINCIPLES = [
  { icon: "people", h: T("The traveller is the integration test", "المتنقّل هو اختبار التكامل"), p: T("If a journey needs three apps and two accounts, the architecture has failed regardless of what the diagram says.", "إن احتاجت الرحلة ثلاثة تطبيقات وحسابين، فقد فشلت البنية مهما قال المخطّط.") },
  ...
  { icon: "eye", h: T("Policy needs evidence", "السياسة تحتاج أدلّة"), p: T("Pricing, access and kerb decisions are simulated against real network state before they are imposed on it.", "تُحاكى قرارات التسعير والوصول والأرصفة على حالة الشبكة الحقيقية قبل فرضها عليها.") },
...
const COVERAGE = [
  { label: T("Public transport", "النقل العام"), items: [T("Bus & metro operations", "تشغيل الحافلات والمترو"), T("Real-time passenger info", "معلومات الركّاب اللحظية"), T("Timetable optimisation", "تحسين الجداول"), T("On-demand transit", "النقل عند الطلب")] },
  ...
  { label: T("Kerb & parking", "الرصيف والمواقف"), items: [T("Smart parking", "المواقف الذكية"), T("Enforcement", "الضبط المروري"), T("Loading zones", "مناطق التحميل"), T("Dynamic pricing", "التسعير الديناميكي")] },
...
const FAQS = [
  ...
  },
  {
    ...
    a: T("Where the network state changes faster than a human control room can react: demand forecasting, disruption rerouting, incident detection from camera and sensor feeds, predictive maintenance on fleets and charging assets, and signal optimisation on congested corridors.", "حيث تتغيّر حالة الشبكة أسرع من قدرة غرفة تحكم بشرية على التفاعل: التنبؤ بالطلب، وإعادة التوجيه عند الاضطراب، وكشف الحوادث من الكاميرات والمستشعرات، والصيانة التنبؤية للأساطيل وأصول الشحن، وتحسين الإشارات في الممرّات المزدحمة."),
  },
];
...
export default function mobility(lang) {
  const hero = heroFor({
    ...
    meta: [
      { k: T("Span", "المدى"), v: T("Traveller, operator, network, kerb", "المتنقّل والمشغّل والشبكة والرصيف") },
...
${section(`
  ${secHead({ kicker: T("Principles", "مبادئ"), h: T("What keeps mobility programmes honest.", "ما يُبقي برامج التنقل صادقة.") }, lang)}
