...
const CAPS = [
  ...
  { icon: "shield", h: T("Insurance intelligence", "ذكاء التأمين"), p: T("Eligibility, prior authorisation and claims workflows with anomaly signals — reducing the paperwork between a patient and an approval.", "سير عمل للأهلية والموافقات المسبقة والمطالبات مع إشارات الشذوذ — بما يقلّص الأوراق بين المريض والموافقة.") },
...
const STACK = [
  ...
  { label: T("Intelligence", "الذكاء"), note: T("Administrative, not diagnostic.", "إداري لا تشخيصي."), items: [T("Scheduling optimisation", "تحسين الجدولة"), T("No-show prediction", "التنبؤ بعدم الحضور"), T("Document processing", "معالجة المستندات"), T("Demand forecasting", "التنبؤ بالطلب"), T("Assistant workflows", "مسارات المساعدين")] },
...
const BOUNDARIES = [
  { icon: "shield", h: T("Administrative AI, clinical humans", "ذكاء إداري وبشر سريريون"), p: T("Our systems optimise scheduling, coordination, documents and operations. Diagnosis and treatment decisions belong to clinicians, and the architecture enforces that boundary.", "تُحسّن أنظمتنا الجدولة والتنسيق والمستندات والعمليات. أما قرارات التشخيص والعلاج فللأطباء، والبنية تفرض هذا الحدّ.") },
  { icon: "key", h: T("Consent is architecture", "الموافقة بنية"), p: T("Who may see what, for which purpose, for how long — modelled explicitly and enforced at the access layer, not in a policy document.", "من يرى ماذا، ولأي غرض، ولأي مدة — يُنمذج صراحةً ويُفرض عند طبقة الوصول، لا في وثيقة سياسات.") },
  { icon: "pin", h: T("Health data stays put", "البيانات الصحية تبقى في مكانها"), p: T("Deployments are architected for the residency and localisation obligations that apply to health data in the jurisdiction.", "تُصمَّم عمليات النشر وفق التزامات الإقامة والتوطين المطبَّقة على البيانات الصحية في النطاق القضائي.") },
