...
const PLATFORMS = [
  ...
  { icon: "radar", h: T("Remote operations", "التشغيل عن بُعد"), p: T("Supervised control from an operations centre, with tele-operation for the exceptions the autonomy stack is not cleared to handle.", "تحكّم مُشرَف عليه من مركز العمليات، مع تشغيل عن بُعد للحالات الاستثنائية غير المصرَّح لطبقة الاستقلالية بمعالجتها.") },
  { icon: "vehicle", h: T("Fleet intelligence", "ذكاء الأساطيل"), p: T("Vehicle and asset telemetry turned into utilisation, condition and predictive maintenance rather than a map of dots.", "تحويل قياسات المركبات والأصول إلى معدّلات استخدام وحالة وصيانة تنبؤية، لا إلى خريطة نقاط.") },
...
const STACK = [
  ...
  { label: T("Autonomy", "الاستقلالية"), note: T("What it can do without an operator.", "ما يمكنه فعله دون مشغّل."), tone: "hi", items: [T("Mission planning", "تخطيط المهام"), T("Navigation", "الملاحة"), T("Obstacle handling", "التعامل مع العوائق"), T("Fail-safe behaviour", "سلوك الأمان عند الفشل"), T("Return-to-base logic", "منطق العودة إلى القاعدة")] },
...
const SAFETY = [
  ...
  { icon: "shield", h: T("Fail-safe behaviour", "سلوك الأمان عند الفشل"), p: T("Loss of link, low power and sensor failure have defined, tested responses — not emergent ones.", "فقد الاتصال وانخفاض الطاقة وعطل المستشعر لها استجابات محدّدة ومُختبَرة، لا استجابات ارتجالية.") },
...
const SCOPE = [
  ...
  { label: T("Integration", "التكامل"), items: [T("Asset management", "إدارة الأصول"), T("Maintenance systems", "أنظمة الصيانة"), T("GIS", "نظم المعلومات الجغرافية"), T("Command centre", "مركز القيادة"), T("Field mobile apps", "تطبيقات الميدان"), T("Digital twin", "التوأم الرقمي")] },
];
...
const FAQS = [
  ...
  },
  {
    ...
    a: T("Autonomy is granted per mission type, not as a general property. A repeated inspection pass over a known site with a cleared route is a good candidate. Anything novel, congested or safety-critical stays under direct supervision with defined fail-safe behaviour.", "تُمنح الاستقلالية لكل نوع مهمة على حدة، لا كخاصية عامة. فالجولة التفتيشية المتكرّرة فوق موقع معروف بمسار معتمد مرشّحة جيدة. أما كل ما هو جديد أو مزدحم أو حسّاس للسلامة فيبقى تحت إشراف مباشر بسلوك أمان محدّد."),
  },
  {
...
${section(`
  ${secHead({
    ...
    lead: T("The hardware is the least differentiated part of an autonomous programme. Everything that determines whether it survives its second year sits in the four bands underneath it.", "العتاد أقلّ أجزاء البرنامج ذاتي التشغيل تميّزًا. وكل ما يحدّد صموده في سنته الثانية يقع في الطبقات الأربع تحته."),
  }, lang)}
