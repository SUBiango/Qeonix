...
const CITY_STACK = [
  ...
  { label: T("Digital experience", "التجربة الرقمية"), note: T("One surface across every city service.", "واجهة واحدة لكل خدمات المدينة."), items: [T("City app", "تطبيق المدينة"), T("Web portal", "البوابة الإلكترونية"), T("Contact centre", "مركز الاتصال"), T("Kiosks", "الأكشاك"), T("Field mobile", "تطبيقات الميدان")] },
  ...
  { label: T("Operation centres", "مراكز العمليات"), note: T("Where the city is actually run.", "حيث تُدار المدينة فعليًا."), items: [T("Command & control", "القيادة والتحكم"), T("Incident management", "إدارة الحوادث"), T("Live operational view", "الصورة التشغيلية المباشرة"), T("Multi-agency coordination", "التنسيق متعدّد الجهات"), T("Emergency support", "دعم الطوارئ")] },
  { label: T("Field operations", "العمليات الميدانية"), note: T("The crews who close the loop.", "الفرق التي تُغلق الحلقة."), items: [T("Crew scheduling", "جدولة الفرق"), T("Mobile work orders", "أوامر العمل المتنقّلة"), T("Evidence capture", "التقاط الأدلّة"), T("Route optimisation", "تحسين المسارات"), T("Completion & QA", "الإنجاز وضبط الجودة")] },
...
const LIFECYCLE = [
  { label: T("A resident reports a fault", "متعامل يبلّغ عن خلل"), icon: "people", note: T("A photo and a location, from the city app or the contact centre.", "صورة وموقع، من تطبيق المدينة أو مركز الاتصال.") },
...
const CAPS = [
  { icon: "radar", h: T("City command centre", "مركز قيادة المدينة"), p: T("A live operational picture across departments, with incident management and multi-agency coordination rather than eight screens showing eight systems.", "صورة تشغيلية مباشرة عبر الإدارات، مع إدارة الحوادث والتنسيق متعدّد الجهات، بدل ثماني شاشات تعرض ثمانية أنظمة.") },
  ...
  { icon: "package", h: T("Field operations", "العمليات الميدانية"), p: T("Crew scheduling, mobile work orders, route optimisation and evidence capture — the part of a smart city programme that is usually left out.", "جدولة الفرق وأوامر العمل المتنقّلة وتحسين المسارات والتقاط الأدلّة — وهو الجزء الذي يُهمَل عادةً في برامج المدن الذكية.") },
...
const REALITY = [
  ...
  { icon: "people", h: T("Design for the crew", "صمّم للفريق الميداني"), p: T("If the mobile work order is worse than the paper it replaced, the programme fails in the field regardless of the platform.", "إن كان أمر العمل المتنقّل أسوأ من الورقة التي حلّ محلّها، فسيفشل البرنامج في الميدان مهما كانت المنصّة.") },
...
const COVERAGE = [
  ...
  { label: T("Waste & cleansing", "النفايات والنظافة"), items: [T("Bin & route optimisation", "تحسين الحاويات والمسارات"), T("Fill-level sensing", "استشعار مستوى الامتلاء"), T("Fleet tracking", "تتبّع الأسطول"), T("Contractor SLA", "اتفاقيات المقاولين"), T("Cleanliness indexing", "مؤشّر النظافة")] },
...
${section(`
  ${secHead({
    ...
    lead: T("Read it downward as a request travelling to the street, and upward as evidence travelling back to the record.", "اقرأها نزولًا كطلب ينتقل إلى الشارع، وصعودًا كأدلّة تعود إلى السجل."),
  }, lang)}
...
${section(`
  ${secHead({ kicker: T("What we have learned", "ما تعلّمناه"), h: T("Why smart city programmes stall.", "لماذا تتعثّر برامج المدن الذكية.") }, lang)}
...
`;
  ...
  return {
    ...
    description: T(
      "A city operating layer from Qeonix: command and control centres, connected infrastructure and IoT, service request lifecycle, field operations, utilities, waste, mobility, parking, EV, environmental monitoring and digital twins — integrated across a mixed multi-vendor estate.",
